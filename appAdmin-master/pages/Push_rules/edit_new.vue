  ...
  <view class="uni-container">
    <uni-forms ref="form" :model="formData" validateTrigger="bind">
      ...
      <uni-forms-item name="survey_id" label="关联问卷" required>
        <uni-data-picker v-model="formData.survey_id" 
		 @change="questionnaireChange"
		collection="questionnaire" field="_id as value, title as text"></uni-data-picker>
      </uni-forms-item>
      ...
      <uni-forms-item name="video_id" label="关联视频" required>
	    ...
	    <uni-forms-item name="questions" label="关联问题">
	  			<uni-data-checkbox :multiple="true" max="10"
				 v-model="formData.questions" :localdata="range" ></uni-data-checkbox>
	  </uni-forms-item>
  ...
  export default {
    ...
    methods: {
      questionnaireChange(e){
      	...
      	if(e&&e.detail&&e.detail.value&&e.detail.value.length>0&&e.detail.value[0].value){
      		this.range = []
      		this.getQuestions(e.detail.value[0].value);
      	}else{
      		this.range = []
      	}
      },
      ...
      async getQuestions(questionnaireId){
      	...
      	const res = await db.collection('questionnaire').doc(questionnaireId).get();
      	console.log(res);
      	const ques = res.result.data[0].questions
      	const selectQues = await db.collection('select_question').get();
      	
      	ques.forEach((a)=>{
      		//只要选择题
      		if(a.question_type==0){
      			selectQues.result.data.forEach((b)=>{
      				if(b._id == a.question_id){
      					this.range.push({
      						value: a.question_id,
      						text: b.content,
      					})
      ...
      submitForm(value) {
		  ...
		  db.collection('Push_records').where({video_id:value.video_id}).remove()
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("serial,surveyNum,survey_id,videoNum,video_id,questions,pushDate,min_score,max_score,pushWeek").get().then((res) => {
          const data = res.result.data[0]
          if (data) {
            this.formData = data
            this.getQuestions(this.formData.survey_id);
          }
        }).catch((err) => {
          uni.showModal({
            content: err.message || '请求服务失败',
