<template>
	<view class="wen-home">
		...
		<view class="wen-box">
			...
			<div class="in">
				<input type="text" class="inp" placeholder="请输入您的ID号" v-model="phone"/>
				</div>
			<view class="itembox" v-for="(item,index) in questionsAnswer" :key="index">
				...
				<view class="writeitem" v-if="item.questionType == 'QUESTION'">
					<textarea  @input="bindTextAreaBlur($event,index)" auto-height
						maxlength="1000" />
				</view>
			...
			<button :disabled="tijiao" type="primary" @click="submitData">提交</button>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			initObj(obj) {
				...
				for (var i = 0; i < this.totalnum; i++) {
					let arr = {
						...
						userAnswer: this.questionsAnswer[i].selectAnswer //每个问题用户选择的答案(逗号拼接)
					}
			...
			bindTextAreaBlur(e,index) {
				this.formSubmitData[index].userAnswer = e.detail.value
				this.countNum()
			},
			//计算已答题目数量
			countNum() {
