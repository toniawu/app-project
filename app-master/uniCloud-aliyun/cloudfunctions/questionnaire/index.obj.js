...
module.exports = {
	...
	async getLatestAnswerTime(user_id) {
		...
		.field({_creatTime:true,num:true})
		...
		if(res.data != ''){
			...
			let wenId = answer.num + ''
			let nowTime = new Date().getTime()/1000
			//如果本次已经作答的问卷，有下一个问卷需要作答，并且下次作答时间大于系统时间，
			// 直接返回系统时间。不允许用户填写序号
			let uqRes = await db.collection(db_user_questionnaire).where(
			{
				user_id:user_id,
				wen_id: wenId,
				next_time : dbc.gt(nowTime),
				next_id:dbc.gt(0)
			}).count()
			if(uqRes.total > 0){
				return  {
	...
	async answerQuestionnaire(answer) {
		...
		delete answer.next_interval_day
		delete answer.next_num
		// 计算分数		
