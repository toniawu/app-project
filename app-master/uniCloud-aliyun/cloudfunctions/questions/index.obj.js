...
module.exports = {
	...
	async getScore() {
		...
		let res = await db.collection('Score').where({uid:this.params.uid}).orderBy('answerTime','desc').get();
		let time = 0;
		if(res.data != ''){
			//获取最后答卷时间
			time  = res.data[0].answerTime
		} 
	...
	async getSurveys() {
		this.params = JSON.parse(this.getHttpInfo().body)
		let res = await db.collection('Surveys').where({num:parseInt(this.params.wen_id)}).get()
		let result = {
			errcode: 200,
			errmsg: '查询成功',
			data: res.data
