...
exports.main = async (event, context) => {
			...
			for (const userGroupData of userGroup.data) {
				...
				if(users){
					...
					if(users.wx_openid){
						...
						const ret = uniSubscribemsg.sendSubscribeMessage({
							touser: openid,
	...
	if(updateUserIds.length>0){
		db.collection('Push_records').where({
					watch_state : 0,
					push_time: dbc.lte(now.getTime()),
					user_id:dbc.in(updateUserIds)
					}).update({
