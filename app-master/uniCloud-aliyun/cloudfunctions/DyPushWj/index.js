...
exports.main = async (event, context) => {
		...
		// //推送未答题的问卷
		const list = await db.collection('uni-id-users').where({_id:'66f123af86206611b48b5234'}).get();
		...
		for (const users of list.data) {
			...
			if(users.wen_id){
				...
				var three  = wendata+3888000000;
	 
				if(three < timestamp || wendata == 0){ 
					//则过期  更新试卷为空
					await db.collection('uni-id-users').where({'_id':users._id}).update({'wen_id':'',wen_date:0});
					await db.collection('Score').where({uid:users._id}).remove()
					await db.collection('Push_records').where({user_id:users._id}).remove()
					if(users.wx_openid){
